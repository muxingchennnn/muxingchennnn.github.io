...
var i = class {
  advance(t2) {
    ...
    let s2 = false;
    if (this.lerp)
      this.value = (o2 = this.value, n2 = this.to, (1 - (l2 = 1 - Math.exp(-60 * this.lerp * t2))) * o2 + l2 * n2), Math.round(this.value) === this.to && (this.value = this.to, s2 = true);
    else {
      this.currentTime += t2;
    ...
    var o2, n2, l2;
    null == (i2 = this.onUpdate) || i2.call(this, this.value, s2), s2 && this.stop();
  }
  stop() {
    this.isRunning = false;
  }
  fromTo(t2, e2, { lerp: i2 = 0.1, duration: s2 = 1, easing: o2 = (t3) => t3, onStart: n2, onUpdate: l2 }) {
    this.from = this.value = t2, this.to = e2, this.lerp = i2, this.duration = s2, this.easing = o2, this.currentTime = 0, this.isRunning = true, null == n2 || n2(), this.onUpdate = l2;
  }
};
var s = class {
...
var o = class {
  ...
  off(t2, e2) {
    var i2;
    this.events[t2] = null == (i2 = this.events[t2]) ? void 0 : i2.filter((t3) => e2 !== t3);
  }
  destroy() {
    this.events = {};
  }
...
var n = class {
  constructor(t2, { wheelMultiplier: i2 = 1, touchMultiplier: s2 = 2, normalizeWheel: n2 = false }) {
    this.onTouchStart = (t3) => {
      ...
      this.touchStart.x = e2, this.touchStart.y = i3, this.lastDelta = { x: s3, y: o2 }, this.emitter.emit("scroll", { deltaX: s3, deltaY: o2, event: t3 });
    }, this.onTouchEnd = (t3) => {
      this.emitter.emit("scroll", { deltaX: this.lastDelta.x, deltaY: this.lastDelta.y, event: t3 });
    }, this.onWheel = (t3) => {
      let { deltaX: i3, deltaY: s3 } = t3;
      this.normalizeWheel && (i3 = e(-100, i3, 100), s3 = e(-100, s3, 100)), i3 *= this.wheelMultiplier, s3 *= this.wheelMultiplier, this.emitter.emit("scroll", { deltaX: i3, deltaY: s3, event: t3 });
    }, this.element = t2, this.wheelMultiplier = i2, this.touchMultiplier = s2, this.normalizeWheel = n2, this.touchStart = { x: null, y: null }, this.emitter = new o(), this.element.addEventListener("wheel", this.onWheel, { passive: false }), this.element.addEventListener("touchstart", this.onTouchStart, { passive: false }), this.element.addEventListener("touchmove", this.onTouchMove, { passive: false }), this.element.addEventListener("touchend", this.onTouchEnd, { passive: false });
  }
  on(t2, e2) {
...
var l = class {
  constructor({ wrapper: e2 = window, content: l2 = document.documentElement, wheelEventsTarget: r = e2, eventsTarget: h = r, smoothWheel: a = true, smoothTouch: c = false, syncTouch: u = false, syncTouchLerp: p = 0.1, __iosNoInertiaSyncTouchLerp: d = 0.4, touchInertiaMultiplier: m = 35, duration: v, easing: g = (t2) => Math.min(1, 1.001 - Math.pow(2, -10 * t2)), lerp: S = !v && 0.1, infinite: w = false, orientation: f = "vertical", gestureOrientation: y = "vertical", touchMultiplier: T = 1, wheelMultiplier: z = 1, normalizeWheel: _ = false, autoResize: M = true } = {}) {
    this.onVirtualScroll = ({ deltaX: e3, deltaY: i2, event: s2 }) => {
      if (s2.ctrlKey)
        return;
      const o2 = s2.type.includes("touch"), n2 = s2.type.includes("wheel");
      if ("both" === this.options.gestureOrientation && 0 === e3 && 0 === i2 || "vertical" === this.options.gestureOrientation && 0 === i2 || "horizontal" === this.options.gestureOrientation && 0 === e3 || o2 && "vertical" === this.options.gestureOrientation && 0 === this.scroll && !this.options.infinite && i2 <= 0)
        return;
      let l3 = s2.composedPath();
      if (l3 = l3.slice(0, l3.indexOf(this.rootElement)), l3.find((t2) => {
        var e4;
        return (null == t2.hasAttribute ? void 0 : t2.hasAttribute("data-lenis-prevent")) || o2 && (null == t2.hasAttribute ? void 0 : t2.hasAttribute("data-lenis-prevent-touch")) || n2 && (null == t2.hasAttribute ? void 0 : t2.hasAttribute("data-lenis-prevent-wheel")) || (null == (e4 = t2.classList) ? void 0 : e4.contains("lenis"));
      }))
        return;
      ...
      s2.preventDefault();
      let r2 = i2;
      "both" === this.options.gestureOrientation ? r2 = Math.abs(i2) > Math.abs(e3) ? i2 : e3 : "horizontal" === this.options.gestureOrientation && (r2 = e3);
      const h2 = o2 && this.options.syncTouch, a2 = o2 && "touchend" === s2.type && Math.abs(r2) > 1;
      a2 && (r2 = this.velocity * this.options.touchInertiaMultiplier), this.scrollTo(this.targetScroll + r2, t({ programmatic: false }, h2 && { lerp: a2 ? this.syncTouchLerp : this.options.__iosNoInertiaSyncTouchLerp }));
    }, this.onNativeScroll = () => {
    ...
    }, window.lenisVersion = "1.0.29", e2 !== document.documentElement && e2 !== document.body || (e2 = window), this.options = { wrapper: e2, content: l2, wheelEventsTarget: r, eventsTarget: h, smoothWheel: a, smoothTouch: c, syncTouch: u, syncTouchLerp: p, __iosNoInertiaSyncTouchLerp: d, touchInertiaMultiplier: m, duration: v, easing: g, lerp: S, infinite: w, gestureOrientation: y, orientation: f, touchMultiplier: T, wheelMultiplier: z, normalizeWheel: _, autoResize: M }, this.animate = new i(), this.emitter = new o(), this.dimensions = new s({ wrapper: e2, content: l2, autoResize: M }), this.toggleClass("lenis", true), this.velocity = 0, this.isLocked = false, this.isStopped = false, this.isSmooth = u || a || c, this.isScrolling = false, this.targetScroll = this.animatedScroll = this.actualScroll, this.options.wrapper.addEventListener("scroll", this.onNativeScroll, { passive: false }), this.virtualScroll = new n(h, { touchMultiplier: T, wheelMultiplier: z, normalizeWheel: _ }), this.virtualScroll.on("scroll", this.onVirtualScroll);
  }
  destroy() {
  ...
  scrollTo(t2, { offset: i2 = 0, immediate: s2 = false, lock: o2 = false, duration: n2 = this.options.duration, easing: l2 = this.options.easing, lerp: r = !n2 && this.options.lerp, onComplete: h = null, force: a = false, programmatic: c = true } = {}) {
    if (!this.isStopped && !this.isLocked || a) {
      ...
      if ("number" == typeof t2) {
        ...
        this.animate.fromTo(this.animatedScroll, t2, { duration: n2, easing: l2, lerp: r, onStart: () => {
          o2 && (this.isLocked = true), this.isScrolling = true;
        }, onUpdate: (t3, e2) => {
          this.isScrolling = true, this.velocity = t3 - this.animatedScroll, this.direction = Math.sign(this.velocity), this.animatedScroll = t3, this.setScroll(this.scroll), c && (this.targetScroll = t3), e2 || this.emit(), e2 && (this.reset(), this.emit(), null == h || h(this), this.__preventNextScrollEvent = true, requestAnimationFrame(() => {
